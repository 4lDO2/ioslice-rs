...
{
    #[inline]
    fn all_uninit(&self) -> &[T::UninitItem] {
    ...
    pub fn is_completely_filled(&self) -> bool {
        self.vectors_filled() == self.vectors_to_fill()
    }
    #[inline]
    pub fn is_partially_filled(&self) -> bool {
        self.vectors_filled() > 0 || self.bytes_filled_for_vector > 0
    }
    #[inline]
    pub fn current_vector_uninit_part(&self) -> Option<&[MaybeUninit<u8>]> {
        let all = self.current_buffer()?;
        ...
        Some(unsafe {
            ...
            let ptr = all.as_ptr().add(self.bytes_filled_for_vector);
            let len = all.len().wrapping_sub(self.bytes_filled_for_vector);
    ...
    pub fn current_vector_init_part(&self) -> Option<&[u8]> {
        ...
        Some(unsafe {
            ...
            let ptr = all.as_ptr();
            let len = self.bytes_filled_for_vector;
    ...
    #[inline]
    fn all_uninit_mut(&mut self) -> &mut [T::UninitItem] {
        self.inner.as_maybe_uninit_slices_mut()
    }

    #[inline]
    pub fn current_vector_mut(&mut self) -> Option<&mut [MaybeUninit<u8>]> {
    ...
    pub fn current_vector_uninit_part_mut(&mut self) -> Option<&mut [MaybeUninit<u8>]> {
        ...
        let (orig_ptr, orig_len) = {
            let all = self.current_vector_mut()?;
        ...
        Some(unsafe {
            ...
            // `uninit_part`.
            let ptr = orig_ptr.add(self.bytes_filled_for_vector);

            let len = orig_len.wrapping_sub(self.bytes_filled_for_vector);
    ...
    /// safe.
    #[inline]
    pub unsafe fn advance_to_end_of_vector(&mut self) {
    }
...
impl<'a> Buffer<&'a mut [u8]> {
    ...
    pub fn from_slice_mut(slice: &'a mut [u8]) -> Self {
        Self {
            bytes_filled_for_vector: slice.len(),
            vectors_filled: 0,
            inner: slice,
        }
    }
